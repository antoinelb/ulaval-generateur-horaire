...
- `docs/conception/` — the design history (original conception documents, deliverable plan, initial ADR, Daniel's request emails). Consult only for extra detail (full grammar specs, worked JSON examples, spike results, rejected-alternative reasoning); where it contradicts `docs/project_plan.md`, the plan wins.
...
- **Two parser grammars** (full specs in `docs/conception/`): program rules ("Règle N – \<contrainte\> parmi :" → `{type: cours|credits, …}` with `sous_groupes`) and préalables (ET/OU trees + `credits_programme`). Anything outside a grammar is kept as raw text (`{"brut": "…"}`) and surfaced, never ignored.
