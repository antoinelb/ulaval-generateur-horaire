...
- **Data files**: `data/cours.json`, one file holding every multi-season `Course` whole, sorted by code (per-course: `code`, `title`, `credits` — a plain number or `{min, max}` for a stage the student weights himself, `cycle`, subject, `prerequisites` raw + parsed tree, contributing programs, `equivalents`, per-season `last_offered` + `options`: each option a *complete* enrolment, so you take one option whole and union its sections' slots; `options: null` = offered but schedule not yet published, distinct from `[]`) — beside the hand-maintained `data/cours.manuel.json` the scraper never writes — and `data/programmes/{code}-{year}.json` — one file per program *and academic year* (`year` is also a field of `core::Program`; a scrape run in May–December files under the current civil year, January–April under the previous one, so students keep the version they enrolled under — ADR `2026-07-annee-de-programme-selon-la-date-de-scrape`), a bare `core::Program` (`credits_required`, `mandatory`, `rules`, `concentrations`, `profiles`, `notes` for prose no grammar covers, `language_requirement` for the course-or-test *exigence linguistique* lifted out of the rules/notes) — beside `data/programmes/{code}.manuel.json` (no year), which carries the hand-encoded `cheminement_type` and is never written by the scraper. Running `ulaval-scraper program` with no URLs refreshes every program already in the directory (slug = filename stem minus the year — ADR `2026-07-programs-sans-url-rafraichit-par-slug`).
