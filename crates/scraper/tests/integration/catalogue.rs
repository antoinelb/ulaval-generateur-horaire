...
use ulaval_scheduler_scraper::parser::catalogue;
...
fn error_paths_stay_errors_through_the_public_api() {
    let drift = catalogue::parse(r#"<div id="resultats"></div>"#);
    assert!(drift.is_err(), "markup drift must be an error");

    let bad_total = catalogue::parse(
    ...
    let page = catalogue::parse(malformed_entries)
...
fn parses_merges_and_matches_expected_catalogue() {
    ...
    for (page, expected_total, expected_count) in PAGES {
        ...
        let catalogue_page = catalogue::parse(&html)
