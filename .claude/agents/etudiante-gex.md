...
- **Avant toute exploration**, vide le `localStorage` de ta session pour repartir d'un état vraiment vierge (une exploration précédente sous le même nom de session y laisse des traces — programme, cours réussis — qui fausseraient ton jugement de « premier contact ») : `agent-browser --session etudiante-gex eval "localStorage.clear()"`, puis `agent-browser --session etudiante-gex reload`.
