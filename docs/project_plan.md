...
- Le `cheminement_type` (A1→H8) n'a aucune source machine-lisible : encodé à la main dans `data/programmes/{code}-{semester}.manuel.json` que le scraper n'écrit jamais — un fichier par millésime, comme le snapshot scrapé qu'il accompagne (ADRs `2026-07-cheminement-type-en-fichier-manuel`, `2026-08-fichier-manuel-de-programme-millesime`) ; format `cheminements_types` (une entrée par variante, le millésime venant du nom du fichier, autorité `core::program_manual`), B-GEX/B-GIN/B-GMC convertis depuis les CSV du dépôt JS, B-GPH en attente d'une vraie source (ADR `2026-08-format-des-cheminements-types-manuels`).
