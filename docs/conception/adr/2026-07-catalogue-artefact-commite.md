# Catalogue : artefact commité et politique d'échec du scrape de listing
...
Le plan ne nommait que `data/cours/{session}.json` et `data/programmes.json` ; le sort du listing (transitoire ? persistant ?) n'était pas décidé, et les fixtures par page de l'ADR `2026-07-cas-de-test-listing-facette-gex` étaient remises en question.

## Décision

- **Le listing est un artefact commité** : `data/catalogue.json`, un seul fichier global, remplacé atomiquement comme les autres snapshots.
...
- **Fixture fusionnée** : `tests/fixtures/test_cases/listing/gex.json` (les 52 cours GEX) remplace les trois fichiers par page.
...
- **Phases séquentielles** : le catalogue est écrit (atomiquement) avant que le scrape des pages cours ne commence ; à ~10 req/s le listing prend ~20 s, le parallélisme n'apporterait rien et coûterait le point de reprise net.
...
- **Listing transitoire (en mémoire)** : un scrape de ~20 min interrompu repartirait de zéro ; le fichier commité est le point de reprise naturel.
