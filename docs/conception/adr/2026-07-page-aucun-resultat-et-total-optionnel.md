# Listing : variante « Aucun résultat » et total optionnel
...
- **Listing sans facette, page au-delà de la fin** (`all_last.html`) : markup entièrement différent — aucun `total-resultats`, aucun `cours-element` ; un seul `<div class="… resultats--offre-etudes"><p>Aucun résultat</p></div>`.
...
- **Quatre fixtures gelées** dans `tests/fixtures/test_cases/listing/` : `gex_0.html` (50 cours), `gex_1.html` (2 cours), `gex_2.html` (0 cours, `total-resultats` présent), `all_last.html` (variante « Aucun résultat »).
- **`ListingPage.total_results` devient `Option<usize>`** : `None` sur la variante « Aucun résultat », qui n'affiche aucun compte.
