...
- **« Le moins de changements possible » est le `seed`** : chaque requête envoie déjà `displayed_placement` comme seed et le cœur trie chaque domaine par distance à lui — proximité sans preuve de minimalité, assumée depuis `2026-07-b-placement-par-satisfaction-fait-main`.
  *Supersédé le 2026-08-28 par `2026-08-b-minimise-la-distance-au-seed` : le rapport UX du 2026-08-27 a montré qu'un ordre de valeurs seul redistribue la moitié du cheminement dès qu'une collision de plafond survient. Le seed reste envoyé de la même façon ; c'est le cœur qui minimise désormais Σ|session − seed| au lieu de rendre la première solution.*
...
- Un objectif de minimalité dans le cœur (nombre de cours déplacés) : fausse précision déjà rejetée par `2026-07-b-placement-par-satisfaction-fait-main` ; le seed suffit.
  *Le comptage de cours déplacés reste rejeté ; c'est la somme des distances qui a été retenue le 2026-08-28 (`2026-08-b-minimise-la-distance-au-seed`), le seed ne suffisant pas.*
