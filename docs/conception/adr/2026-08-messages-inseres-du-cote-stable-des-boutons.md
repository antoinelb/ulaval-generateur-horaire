...
`min-height: 4.5rem` réserve la hauteur mais pas l'alignement : sans `align-content`, `flex-wrap` répartit les rangées sur toute la hauteur réservée, et un repli à une seule ligne semblait flotter au centre du bloc plutôt que de rester ancré en haut — `.grid-head` gagne `align-content: flex-start`.
Le statut (« combinaison automatique - sans conflit ✓ ») n'avait ni plafond de largeur ni troncature : sur les largeurs où le groupe retombe sur deux lignes, un statut long pousse les boutons d'export hors de la ligne plutôt que de se tronquer lui-même — `.grid-status` gagne `min-width: 0; flex: 0 1 auto` (il cède le premier) avec troncature par ellipse, le texte complet restant lisible en `title` ; `.grid-head .grid-share` gagne `flex: none` pour que les boutons ne rétrécissent jamais.
La classe `grid-export` posée sur les deux boutons d'export n'a jamais été stylée (aucune règle ne la cible) : retirée.
