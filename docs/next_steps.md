...
- [x] Câblage `ui` (jalon 7+ ; hors cœur) : `core` piloté depuis Dioxus, snapshot chargé au navigateur — **aucune règle métier dans la vue** (invariant) : couverture des règles, candidats et validation viennent tous de `core`. Lire `.claude/dioxus.md` avant tout code Dioxus 0.7
    - Verify : ✅ 2026-08-13 — l'interface complète des jalons 3–9 tourne (`make ui`) : horaire hebdomadaire avec fantômes/conflits/épinglage, panneau de règles à deux onglets, organigramme proposé/vérifié par `place` dans un Web Worker (ADR `2026-08-crate-ui-calculations-et-worker` ; le crate a depuis fusionné dans `crates/wasm`, ADR `2026-08-fusion-des-crates-wasm-et-ui-calculations`), partage par URL et cours manuels ; modules purs à 100 % de couverture, vue wasm32-only (ADR `2026-08-architecture-du-crate-ui`). Reste du jalon 10 : classement par préférences (modèle à concevoir contre des données réelles)
...
- ~~**Interaction règles × profils**~~ : tranchée le 2026-08-19 — deux menus « Concentration »/« Profil » au panneau (défaut : première concentration, jamais de profil imposé), et le solveur voit la portée choisie (obligatoires du bloc dans `course_list`, bassin d'injection et crédits en sus scopés) ; ADR `2026-08-selection-concentration-et-profil-au-panneau`.
...
- ~~**`credits_in_addition` inerte**~~ : lu désormais par `wasm::credits::credit_summary` — l'en-tête affiche « N/120 cr au bac », crédits *en sus* et préuniversitaires tenus à part (notes 2026-08-13).
- **Changer de programme garde les cours placés de l'ancien** (rapport étudiante-cegep 2026-08-19, reproduit deux fois) : « changer » → choisir B-GEX conserve la grille B-GMC entière, comptée silencieusement (« 114/120 cr » avec des cours GMC) — un seul `Plan` pour tous les programmes, là où la version JS garde un instantané par (programme, millésime). À trancher : purge au changement (annulable) ou instantané par programme.
- **Les défauts B-GMC font échouer « Proposer »** (même rapport) : 8 sessions à 15 cr n'offrent aucune marge pour 120 cr + stage en sus — premier clic systématiquement « rien trouvé », sans dire quel réglage poser en premier. Défauts par programme (`credits_required`/plafond) ou message qui nomme le levier ?
- **« IFT 10426 » dans un toast de présomption** : coquille de la page source (préalable brut de MAT-2910), gardée en `raw` comme voulu — corrigeable par millésime dans `cours.manuel.json` si souhaité.
