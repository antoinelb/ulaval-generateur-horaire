...
- [x] Câblage `ui` (jalon 7+ ; hors cœur) : `core` piloté depuis Dioxus, snapshot chargé au navigateur — **aucune règle métier dans la vue** (invariant) : couverture des règles, candidats et validation viennent tous de `core`. Lire `.claude/dioxus.md` avant tout code Dioxus 0.7
    - Verify : ✅ 2026-08-13 — l'interface complète des jalons 3–9 tourne (`make ui`) : horaire hebdomadaire avec fantômes/conflits/épinglage, panneau de règles à deux onglets, organigramme proposé/vérifié par `place` dans un Web Worker (`crates/ui-calculations`, ADR `2026-08-crate-ui-calculations-et-worker`), partage par URL et cours manuels ; modules purs à 100 % de couverture, vue wasm32-only (ADR `2026-08-architecture-du-crate-ui`). Reste du jalon 10 : classement par préférences (modèle à concevoir contre des données réelles)
...
- ~~**`credits_in_addition` inerte**~~ : lu désormais par `ui-calculations::credit_summary` — l'en-tête affiche « N/120 cr au bac », crédits *en sus* et préuniversitaires tenus à part (notes 2026-08-13).
